...
const PublicTeamRosterContext =
	createContext<PublicTeamRosterContextValue | null>(null);

function normalizedRoles(roles: PlayerRole[] | undefined) {
	return (roles?.filter(Boolean) ?? []).filter((r): r is PlayerRole =>
		Object.hasOwn(LANE_ROLE_LABELS, r),
	);
}
...
export function PublicTeamRosterModalProvider({
	children,
}: {
	children: ReactNode;
}) {
