...
export const Route = createFileRoute("/legacy/app/auth/login")({
  component: RouteComponent,
  beforeLoad: () => {
    if (typeof window !== "undefined" && pb.authStore.isValid) {
      const id = pb.authStore.record?.id;
      if (id) throw redirect({ to: "/legacy/app/$id/", params: { id } } as never);
    }
  },
});
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        if (record?.id) {
          await navigate({ to: "/legacy/app/$id/", params: { id: record.id } } as never);
        }
      } catch (err) {
        toast.error(
