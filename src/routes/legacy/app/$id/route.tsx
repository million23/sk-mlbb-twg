...
export const Route = createFileRoute("/legacy/app/$id")({
  component: AdminLayout,
  beforeLoad: ({ params }) => {
    if (typeof window !== "undefined" && !pb.authStore.isValid) {
      throw redirect({ to: "/legacy/app/auth/login" });
    }
    const id = (params as { id?: string })?.id;
    const recordId = pb.authStore.record?.id;
    if (id && recordId && id !== recordId) {
      throw redirect({ to: "/legacy/app/$id/", params: { id: recordId } } as never);
    }
  },
});

const primaryNavItems = [
  { to: "/legacy/app/$id", label: "Dashboard", icon: LayoutDashboard },
  { to: "/legacy/app/$id/participants", label: "Participants", icon: Users },
  { to: "/legacy/app/$id/teams", label: "Teams", icon: UsersRound },
  { to: "/legacy/app/$id/team-standing", label: "Team Standing", icon: ListOrdered },
  { to: "/legacy/app/$id/tournament", label: "Tournament", icon: Trophy },
  { to: "/legacy/app/$id/matches", label: "Matches", icon: Swords },
] as const;

/** Above the user menu in the sidebar footer. */
const footerNavItems = [
  { to: "/legacy/app/$id/audit-logs", label: "Audit log", icon: ScrollText },
  { to: "/legacy/app/$id/admins", label: "Admins", icon: ShieldCheck },
] as const;
...
function AdminLayoutContent() {
  ...
  const visibleFooterNavItems = footerNavItems.filter(
    (item) => item.to !== "/legacy/app/$id/audit-logs" || canAudit,
  );
  const {
    setOpenMobile,
    isMobile,
  ...
  const handleSignOut = () => {
    ...
    navigate({ to: "/legacy/app/auth/login" });
  ...
  const isActive = (path: string) => {
    ...
    if (path === "/legacy/app/$id" || path === "/legacy/app/$id/") {
      return pathNorm === `/legacy/app/${id}`;
    }
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton
                ...
                isActive={isActive("/legacy/app/$id/")}
                    ...
                    <SidebarMenuButton
                      render={
                        <Link
                          ...
                          params={{ id }}
                          activeOptions={{ exact: item.to === "/legacy/app/$id" }}
                          onClick={closeMobileSidebar}
                        />
                      }
