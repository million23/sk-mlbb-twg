...
export const Route = createFileRoute("/legacy/app/$id/team-standing")({
...
function TeamStandingPage() {
		...
		for (const lane of LANE_ORDER) {
			...
			const laneRows = candidates.map((candidate) => {
				const avgPerformanceRating =
					candidate.perfCount > 0
						? candidate.perfTotal / candidate.perfCount
						: 0;
				const avgKda =
					candidate.kdaCount > 0 ? candidate.kdaTotal / candidate.kdaCount : 0;
				const avgGold =
					candidate.goldCount > 0
						? candidate.goldTotal / candidate.goldCount
						: 0;
				const score =
	...
	return (
		<div className="flex flex-col gap-6">
			...
			{!tournamentId || !tournamentEligible ? null : teamsLoading ||
				matchesLoading ||
				matchResultsLoading ? (
				<Card>
				...
				<SheetContent className="data-[side=right]:w-[96vw] data-[side=right]:max-w-[96vw] data-[side=right]:sm:w-[900px] data-[side=right]:sm:max-w-[900px] overflow-auto">
					...
					<div className="px-4 pb-3">
						<RadioGroup
							...
							onValueChange={(v) =>
								setBestLaneFilter((v ?? "all") as "all" | LaneKey)
							}
					...
					{filteredBestLaners.length < 1 ? (
						...
						<div className="px-4 pb-4">
							<div className="mb-3 rounded-md border border-border/60 bg-muted/20 p-3 text-xs text-muted-foreground">
								...
								{BEST_LANER_SCORE_WEIGHTS.teamWinRate}) + (normalized lane
								sample size x {BEST_LANER_SCORE_WEIGHTS.sampleSize}). Higher
								score ranks first.
							</div>
							<Table>
								<TableHeader>
