...
import { useArchivedTeams, useTeams } from "@/hooks/legacy/use-teams";
import {
...
import { buildTeamLaneSuggestionsByParticipant } from "@/lib/legacy/team-lane-recommendations";
import {
	isValidPhilippineMobile,
	sanitizePhilippineMobileInput,
} from "@/lib/legacy/philippine-mobile";
...
import {
	...
	toTitleCaseWords,
} from "@/lib/legacy/participant-normalize";
...
type ParticipantsViewKey =
	`${ParticipantSortMode}${typeof PARTICIPANTS_VIEW_KEY_SEP}${ParticipantTeamAgeFilter}`;
...
function participantMatchesTeamAgeFilter(
	...
	const want: "under18" | "18+" = filter === "under18" ? "under18" : "18+";
	if (!p.team?.trim()) {
...
export const Route = createFileRoute("/legacy/app/$id/participants/")({
...
function ParticipantsPage() {
	...
	const suggestionsByParticipant = useMemo(() => {
		...
		for (const [pid, list] of raw) {
			map.set(
				pid,
				list.map(
					({ suggestedTeamId, suggestedTeamName, suggestionPriority }) => ({
						suggestedTeamId,
						suggestedTeamName,
						suggestionPriority,
					}),
	...
	const [participantsViewKey, setParticipantsViewKey] =
		useState<ParticipantsViewKey>(() =>
			buildParticipantsViewKey("default", "all"),
		);
	...
	const filteredParticipants = useMemo(() => {
		...
		return participants.filter((p) => {
			...
			if (
				!participantMatchesTeamAgeFilter(p, teamAgeFilter, teamMajorityByTeamId)
			) {
				return false;
			}
			return true;
												...
												<SelectValue placeholder="Sort and age group">
													{(v: unknown) =>
														...
														v.includes(PARTICIPANTS_VIEW_KEY_SEP)
															? participantsViewKeyLabel(
																	v as ParticipantsViewKey,
																)
															: "Sort and age group"
													}
												</SelectValue>
