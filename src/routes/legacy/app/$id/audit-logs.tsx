...
import { FxAuditSearch, FxAuditTable } from "@/lib/legacy/loading-placeholders";
import { useInView } from "@/hooks/legacy/use-in-view";
import {
  useAuditLogInfinite,
  type AuditLogRow,
} from "@/hooks/legacy/use-audit-log";
import { useAdmins } from "@/hooks/legacy/use-admins";
import { buildAdminNameByIdMap } from "@/lib/legacy/audit-actor-display";
...
export const Route = createFileRoute("/legacy/app/$id/audit-logs")({
  beforeLoad: ({ params }) => {
    if (typeof window === "undefined") return;
    const id = (params as { id?: string }).id;
    if (!id) return;
    if (!canViewAuditLog(pb.authStore.record as { role?: string } | null)) {
      throw redirect({ to: "/legacy/app/$id/", params: { id } } as never);
    }
  },
