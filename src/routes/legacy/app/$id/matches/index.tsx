...
} from "@/lib/legacy/loading-placeholders";
...
export const Route = createFileRoute("/legacy/app/$id/matches/")({
...
function MatchesPage() {
	...
	const [matchStatsMatch, setMatchStatsMatch] = useState<MatchRecord | null>(
		null,
	);
	...
	const teamsForAutoMatches = useMemo(
		() =>
			(teams ?? []).filter(
				(t) => t.archived !== true && t.status !== "inactive",
			),
		[teams],
	);
	...
	const filteredMatchesByAge = useMemo(() => {
		...
		return matches.filter((m) => {
			...
			return (
				teamABracket === matchesAgeCategory &&
				teamBBracket === matchesAgeCategory
			);
		});
	...
	const handleAutoMatches = useCallback(() => {
		...
		const payload = autoMatchPreview.rows.map((preview, index) => {
			...
			return {
				...
				bestOf: Math.max(
					1,
					Number.isFinite(preview.bestOf) ? preview.bestOf : 3,
				),
					...
					{tournamentsLoading ? (
						...
						<Combobox
							...
							filter={(itemId, query) => {
								...
								return (
									label.includes(q) || slug.includes(q) || rawTitle.includes(q)
								);
							}}
							autoHighlight
						>
					...
					<CardContent>
						<div className="flex flex-wrap gap-2">
							...
							<Button
								...
								size="sm"
								variant={
									matchesAgeCategory === "under18" ? "default" : "outline"
								}
			...
			<AlertDialog open={!!archiveId} onOpenChange={() => setArchiveId(null)}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							The match will be hidden from the schedule until you restore it
							from Archived matches. Related{" "}
							<code className="text-xs">match_drafts</code> rows are not deleted
							automatically.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
				...
				>
					<DrawerContent className="max-h-[85vh] flex w-full flex-col overflow-hidden">
						<DrawerHeader className="shrink-0 px-4 text-left">
							...
							<DrawerDescription>
								Pick an age bracket. Teams are included only if more than half
								of their members are in that bracket.
							</DrawerDescription>
						</DrawerHeader>
						<div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4">
							<div className="flex flex-col gap-2">
								...
								<p className="text-xs text-muted-foreground">
									Eligible teams: {autoMatchBracketTeams.length}. Teams without
									a strict majority are skipped.
								</p>
							</div>
						</div>
						<DrawerFooter className="shrink-0 border-t border-border px-4 pt-3 pb-4">
							<Button
								...
								disabled={
									mutations.createMany.isPending ||
									autoMatchBracketTeams.length < 2
								}
							>
								Preview matches
							</Button>
					...
					<DrawerContent className="flex max-h-[92vh] w-full flex-col overflow-hidden">
									...
									{autoMatchPreview.leftOut ? (
										<div className="rounded-lg border border-border/70 bg-muted/20 px-3 py-2 text-sm">
											...
											<span className="font-medium">
												{autoMatchPreview.leftOut.name ??
													autoMatchPreview.leftOut.id}
											</span>
										</div>
									) : null}
								</div>
						...
						<DrawerFooter className="shrink-0 border-t border-border px-4 pt-3 pb-4">
							...
							<Button
								...
								variant="outline"
								onClick={() =>
									setAutoMatchPreview(buildAutoMatchPreview(autoMatchBracket))
								}
								disabled={mutations.createMany.isPending}
							>
								<Shuffle className="size-4" />
					...
					>
						<SelectTrigger className="w-full">
							<SelectValue placeholder="Status">
								{(value) =>
									value
										? (MATCH_STATUSES.find((x) => x.value === value)?.label ??
											String(value))
										: null
								}
							</SelectValue>
