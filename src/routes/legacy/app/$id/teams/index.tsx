...
import {
	...
	useParticipants,
} from "@/hooks/legacy/use-participants";
import {
	useArchivedTeams,
	useTeamMutations,
	useTeams,
} from "@/hooks/legacy/use-teams";
...
import type { Collections, PlayerRole } from "@/types/__pocketbase-types";
...
export const Route = createFileRoute("/legacy/app/$id/teams/")({
	component: TeamsPage,
});
...
function QuickTeamFromUnassignedContent({
	...
	const canContinue =
		...
		selectedIds.size >= 1 &&
		selectedIds.size <= QUICK_TEAM_MAX_MEMBERS;
				...
				<div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain">
					<div className="flex flex-col gap-4 pb-1">
						<DialogDescription className="text-left">
							You are about to create this team and assign the selected players
							in one step. This cannot be undone from here (you can edit the
							team or remove members afterward).
						</DialogDescription>
						...
						{suggestedLaneIds?.length ? (
							<>
								<p>
									...
									lane). Each time you open Quick team (or use the button
									below), a different valid mix is chosen when more than one
									exists. You can change the roster later on the team.
								</p>
								{!selectionMatchesLaneSuggestion ? (
									<div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
										<p>
											Pick another lane-balanced five, or re-check the boxes
											above.
										</p>
										<Button
											type="button"
							...
							<>
								<p>
									Select up to {QUICK_TEAM_MAX_MEMBERS} unassigned players. You
									can add or remove members later.
								</p>
								<p>
...
function TeamsLoadingSkeleton({ layout }: { layout: "table" | "cards" }) {
...
function TeamsPage() {
	...
	const handleArchiveConfirm = useCallback(() => {
		...
		const displayName = teams?.find((t) => t.id === id)?.name?.trim() || "Team";
	...
	useEffect(() => {
		setQuickTeamCaptain((c) => (c && !quickTeamSelected.has(c) ? "" : c));
	}, [quickTeamSelected]);
		...
		const baseColumns: SpreadsheetColumn<T>[] = [
			...
			},
			{
				...
				get: (t) => {
					...
					return members
						.map(
							(m) => formatParticipantNameDisplay(m.name) || m.gameID || m.id,
						)
						.join("; ");
				},
			},
						...
						<DialogHeader className="shrink-0 flex flex-col gap-1.5 text-left">
							...
							{quickTeamStep === 1 ? (
								<DialogDescription>
									...
									players, then confirm to create the team and assign everyone
									at once.
								</DialogDescription>
							) : null}
						</DialogHeader>
