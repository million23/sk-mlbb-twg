...
const PUBLIC_UNAVAILABLE = "/legacy/p/not-available" as const;

export const Route = createFileRoute("/legacy/p")({
...
const nav = [
  { to: "/legacy/p/tournaments" as const, label: "Tournaments", icon: Trophy },
  { to: "/legacy/p/standing" as const, label: "Standing", icon: ListOrdered },
  { to: "/legacy/p/teams" as const, label: "Teams", icon: UsersRound },
  { to: "/legacy/p/participants" as const, label: "Participants", icon: Users },
];

function PublicShell() {
  const pathname = useLocation({ select: (l) => l.pathname });
  /** Keep list queries subscribed for the whole /legacy/p subtree so navigation does not drop observers and GC refetch windows. */
