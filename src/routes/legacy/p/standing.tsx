...
export const Route = createFileRoute("/legacy/p/standing")({
...
function PublicStandingPage() {
	...
	const matchesForRound = useMemo(
		() =>
			roundFilter === "all"
				? (matches ?? [])
				: (matches ?? []).filter(
						(m) => (m.round?.trim() || "Bracket") === roundFilter,
					),
		[matches, roundFilter],
	);
	...
	const standings = useMemo(() => {
		...
		for (const p of participants ?? []) {
			...
			if (!tid) continue;
			const current = membersByTeam.get(tid) ?? {
				under18: 0,
				adults: 0,
				total: 0,
			};
	...
	const bestLaners = useMemo(() => {
		if (!matchesForRound.length || !matchResults?.length)
			return [] as BestLanerRow[];
		const eligibleMatchIds = new Set(
			matchesForRound
				.filter((m) => Boolean(m.teamA) && Boolean(m.teamB))
				.map((m) => m.id),
		);
		type Candidate = {
			lane: LaneKey;
		...
		for (const lane of LANE_ORDER) {
			...
			const maxSamples = candidates.reduce(
				(max, c) => Math.max(max, c.playerMatchResults),
				1,
			);
			const laneRows = candidates.map((c) => {
				const avgPerformanceRating =
					c.perfCount > 0 ? c.perfTotal / c.perfCount : 0;
				...
				const score =
					...
					(c.playerMatchResults / maxSamples) *
						BEST_LANER_SCORE_WEIGHTS.sampleSize;
				return {
					lane: c.lane,
					laneLabel: LANE_LABELS[c.lane],
					playerId: c.playerId,
					playerLabel: c.playerLabel,
					teamName: c.teamName,
					teamWins: c.teamWins,
					teamLosses: c.teamLosses,
					teamWinRate: c.teamWinRate,
					playerMatchResults: c.playerMatchResults,
			...
			laneRows.sort((a, b) => {
				...
				if (b.teamWinRate !== a.teamWinRate)
	...
	const filteredBestLaners = useMemo(
		() =>
			bestLaneFilter === "all"
				? bestLaners
				: bestLaners.filter((r) => r.lane === bestLaneFilter),
					...
					>
						<SelectTrigger className="h-11 w-full rounded-xl border-border/80 bg-card/40 shadow-inner">
							<SelectValue placeholder="All rounds">
								{(value: string | null) =>
									value && value !== "all" ? value : "All rounds"
								}
							</SelectValue>
						</SelectTrigger>
						<SelectContent>
					...
					<Card className="overflow-hidden border-border/80 bg-card/50">
						...
						<CardContent className="overflow-x-auto p-0">
							{filteredStandings.filter(
								(r) => r.played > 0 || r.matchWins > 0 || r.gameWins > 0,
							).length === 0 ? (
								...
								<Table>
									<TableHeader>
										<TableRow className="border-border/50">
											...
											<TableHead className="text-right">
												{headerWithTooltip("GD", "Game difference")}
											</TableHead>
											<TableHead className="text-right pr-6">
												{headerWithTooltip("Win %", "Match win rate")}
											</TableHead>
										</TableRow>
									</TableHeader>
									<TableBody>
					...
					{bestLaners.length > 0 && (
						<Card className="overflow-hidden border-border/80 bg-card/50">
							<CardHeader className="pb-3">
								...
								<p className="text-sm text-muted-foreground">
									Ranked by performance rating, KDA, team win rate, and lane
									match count.
								</p>
								{/* Lane filter */}
								<fieldset className="flex flex-wrap gap-2 border-0 m-0 p-0 pt-1">
									<legend className="sr-only">Filter by lane</legend>
									{(["all", ...LANE_ORDER] as ("all" | LaneKey)[]).map(
										(lane) => (
											<button
												key={lane}
												type="button"
									...
									<Table>
										<TableHeader>
											<TableRow className="border-border/50">
												...
												<TableHead className="text-right">
													{headerWithTooltip("Avg KDA", "Average KDA ratio")}
												</TableHead>
												<TableHead className="text-right">
													{headerWithTooltip(
														"Avg Gold",
														"Average accumulated gold",
													)}
												</TableHead>
												<TableHead className="text-right">Team W</TableHead>
												<TableHead className="text-right">Team L</TableHead>
												<TableHead className="text-right">
													{headerWithTooltip("Win %", "Team match win rate")}
												</TableHead>
												<TableHead className="text-right pr-6">
													{headerWithTooltip("Matches", "Lane match count")}
												</TableHead>
											</TableRow>
										</TableHeader>
										<TableBody>
